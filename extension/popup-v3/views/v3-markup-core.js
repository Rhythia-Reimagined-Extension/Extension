...
function overview() {
  const groups = {
    profile: ['advancedStats', 'playerCompare', 'titleProgression'],
...
function proxyControl(id, label, type = 'select', options = []) {
  const fallback = id === 'option-scoreCards-scoresView' ? 'grid' : id === 'option-titleProgression-crownMode' ? '3d' : '';
...
function choiceControl(id, label, options) {
  const fallback = id === 'option-scoreCards-scoresView' ? 'grid' : '3d';
  const parsed = v3ParseModuleOptionId(id);
...
function moduleScreen(key) {
  const item = MODULES[key];
  let body = '';
  if (key === 'advancedStats') body = `<section class="v3-card"><div class="v3-card-title"><strong>Profile sections</strong><span>${isEnabled(key) ? 'Enabled' : 'Module disabled'}</span></div>${checkControl('option-advancedStats-ratingProfile', 'Rating Profile')}${checkControl('option-advancedStats-tempoProfile', 'Tempo Profile')}${proxyControl('option-advancedStats-profileStyle', 'Profile layout', 'select', [['soft-blocks', 'Soft Blocks'], ['profile-surface', 'Profile Surface'], ['pill-rows', 'Pill Rows']])}</section><section class="v3-card"><div class="v3-card-title"><strong>Stats cards</strong><span>Profile progress</span></div><p class="v3-helper">Raw RP, Weighted RP, FC Count, Play Count and other stat rows.</p>${proxyControl('history-inline-stats', 'Compare against', 'select', [['firstSnapshotToday', 'Open day: first snapshot'], ['previousCapture', 'Open day: previous snapshot'], ['previousDayClose', 'Closed day: previous day close']])}</section>`;
  if (key === 'playerCompare') body = '<section class="v3-card"><div class="v3-card-title"><strong>Player Compare</strong><span>Side-by-side context</span></div><p class="v3-helper">Compare player performance, map results and Rhythm Points in one focused view.</p><div class="v3-info-callout"><strong>Included</strong><span>Map comparison, Rhythm Points comparison and score weighting.</span></div></section>';
   if (key === 'scoreCards') body = `<section class="v3-card"><div class="v3-card-title"><strong>Score Cards</strong><span>Profile and /scores</span></div>${checkControl('option-scoreCards-customCards', 'Use custom Score Cards (same design on /profile)')}${checkControl('option-scoreCards-userScoresWeight', 'Show User Scores Weight')}${checkControl('option-scoreCards-spinScores', 'Show Spin Scores')}${checkControl('option-scoreCards-watchReplay', 'Show watch replay button')}${choiceControl('option-scoreCards-scoresView', 'Scores page (/scores) layout', [['list', 'List'], ['grid', 'Grid']])}</section>`;
  if (key === 'statHistory') body = `<section class="v3-card"><div class="v3-card-title"><strong>What it does</strong><span>Closed-day records</span></div><p class="v3-helper">Click a stat on the profile to see one final value per completed calendar day.</p><div class="v3-info-callout"><strong>Collection</strong><span>Retention and saved data are configured in History &amp; Data.</span></div></section><section class="v3-card"><div class="v3-card-title"><strong>Expanded stat history</strong><span>Profile display</span></div><p class="v3-helper">Choose how much history appears after clicking a profile stat.</p>${proxyControl('history-display-mode', 'Show records', 'select', [['latestOpenAndClosed', 'Latest open state + closed days'], ['closedOnly', 'Closed days only'], ['allSnapshots', 'All open-day snapshots + closed days'], ['firstSnapshotAndClosed', 'First open-day snapshot + closed days']])}${proxyControl('history-grouping', 'Group records by', 'select', [['daily', 'Daily'], ['weekly', 'Weekly'], ['monthly', 'Monthly']])}</section>`;
  if (key === 'rankingHistory') body = `<section class="v3-card"><div class="v3-card-title"><strong>What it does</strong><span>Independent collection</span></div><p class="v3-helper">Stores the last Global Rank, Country Rank and Rhythm Points state for each observed day.</p><div class="v3-info-callout"><strong>Separate baseline</strong><span>Ranking history is collected independently from Stats history.</span></div></section><section class="v3-card"><div class="v3-card-title"><strong>Ranking cards</strong><span>Profile progress</span></div><p class="v3-helper">Global, Country and Rhythm Points use their own baseline.</p>${proxyControl('history-inline-ranking', 'Compare against', 'select', [['previousDayClose', 'Closed day: previous day close'], ['firstSnapshotToday', 'Open day: first snapshot'], ['previousCapture', 'Open day: previous snapshot']])}</section>`;
