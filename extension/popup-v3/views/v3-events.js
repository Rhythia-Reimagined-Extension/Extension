...
screen.addEventListener('click', event => {
  ...
  if (dataView) { state.dataView = dataView.dataset.v3DataView; render(); return; }
   ...
   if (!action) return;
    ...
    if (action === 'cancel-profile-import') { ++state.profileImportSelectionGeneration; state.profileImport = null; render(); return; }
    if (action === 'confirm-profile-import') { confirmProfileImport(); return; }
    if (action === 'backup-choose') { chooseV3LocalBackupFolder().catch(error => setV3Status(error?.message || 'Local backup setup was not completed')); return; }
    ...
    if (action === 'archive-back') { state.archive = null; state.dataView = 'backup'; render(); return; }
